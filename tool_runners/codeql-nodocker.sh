...
while [[ $# -gt 0 ]]; do
  case $1 in
    --tech)
      ...
      ;;
    --dotnet-build)
      BUILD_COMMAND="--command='dotnet build'"
      shift # past argument
      ;;
    --*|-*)
...
if ! { [ "$TECH" = "java" ] || [ "$TECH" = "csharp" ] || [ "$TECH" = "python" ] || [ "$TECH" = "go" ]; } then
...
if [[ -v BUILD_COMMAND ]]; then
    ARGS="$ARGS $BUILD_COMMAND"
fi

eval "$CODEQL database create codeql_db --language=$TECH $ARGS" >&2
